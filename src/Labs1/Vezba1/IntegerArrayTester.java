...
final class IntegerArray{
    ...
    IntegerArray getSorted(){
        int [] sortedNums = Arrays.copyOf(this.numbers,length());
        Arrays.sort(sortedNums);
        return new IntegerArray(sortedNums);
    }
    IntegerArray concat(IntegerArray ia){
        int size = this.length() + ia.length();
        int [] newArray = new int[size];
        System.arraycopy(this.numbers,0,newArray,0,this.length());
        System.arraycopy(ia.numbers,0,newArray,this.length(),ia.length());
